...
class Filter extends React.Component{
    ...
    handleLocationChange = (loc) => {
        const locationId = loc.target.value;

        const { mealtype,locations } = this.state;
        const selectedLocation = locations.find((item) => item.city_id === (locationId));
        ...
        axios({
            ...
            .then(res => {
            this.setState({ restaurant: res.data.restaurants, selectedLocation })
            })
            .catch(err => console.log(err))
        ...
        return(
            <div>
                ...
                <div className="container mb-5">
                    ...
                    { mealtype ? 
                        <h2 className="filter-heading mt-3"> {mealtype ? `${mealtype} Places in ${this.state.selectedLocation ? this.state.selectedLocation.name : 'some location'}` : 'some Places in Mumbai'}</h2>
                        :
                        <h2 className="filter-heading mt-3"> some Places in Mumbai</h2>
                    }
